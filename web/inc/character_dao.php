...
class CharacterDAO {
  ...
  function getCharacterByUnicode($unicode) {
    ...
    $query = "SELECT unicode, c, pinyin, radical, strokes, other_strokes, english, notes, type " .
             "FROM characters " .
    ...
    if ($row = $databaseUtils->fetch_array($result)) {
      $character = new CharacterModel($row[0], 
                                      $row[1], 
                                      $row[2], 
                                      $row[3], 
                                      $row[4], 
    ...
    if (isset($unicode2)) {
      ...
      $result =& $databaseUtils->executeQuery($query);
      if ($row = $databaseUtils->fetch_array($result)) {
        $diacritic = new CharacterModel($row[0], 
                                        $row[1], 
                                        $row[2], 
                                        $row[3], 
                                        $row[4], 
  ...
  /**
    * Gets a character by value or Unicode
    * @return A Character object
    */
  ...
  function getCharactersByRadicals($radical) {
    ...
    while ($row = $databaseUtils->fetch_array($result)) {
      $characters[] = new CharacterModel($row[0], 
                                         $row[1], 
                                         $row[2], 
                                         $row[3], 
                                         $row[4], 
    ...
    for ($i = 0; $i<$len; $i++) {
      ...
      if ($row = $databaseUtils->fetch_array($result)) {
        ...
        if (isset($type) && (($type == 'ipa_diacritic') || ($type == 'dev_diacritic')) && (count($characters) > 0)) {
          $characters[count($characters)-1]->setDiacritic(new CharacterModel($row[0], // unicode
                                                                             $row[1], // c
                                                                             $row[2], // pinyin
                                                                             $row[3], // radical
                                                                             $row[4], // strokes
          ...
          $characters[] = new CharacterModel($row[0], // unicode
                                             $row[1], // c
                                             $row[2], // pinyin
                                             $row[3], // radical
                                             $row[4], // strokes
        ...
        $val = new Character($c);
        $u = $val->getIntCode();
        $characters[] = new CharacterModel($u, $c, "", "", 0, 0, "", "", "unknown", NULL);
      }
    }
