...
class RadicalDAO {
  ...
  function getAllRadicals() {
    ...
    $query = "SELECT id, traditional, simplified, pinyin, strokes, simplified_strokes, other_forms, english " .
             "FROM radicals " .
             "ORDER BY id";
    //error_log("getAllRadicals, query: " . $query);
    $result =& $databaseUtils->executeQuery($query);
    while ($row = $databaseUtils->fetch_array($result)) {
      $radicals[] = new Radical($row[0], $row[1], $row[2], $row[3], $row[4], 
                                $row[5], $row[6], $row[7]);
    }
  ...
  function getRadical($c) {
    ...
    if ($row = $databaseUtils->fetch_array($result)) {
      $radical = new Radical($row[0], $row[1], $row[2], $row[3], $row[4], $row[5], 
                             $row[6], $row[7] );
    }
