...
class MeasureWordDAO {
    ...
    function getAllMeasureWords() {
        ...
        $query = "SELECT DISTINCT id, simplified, traditional, pinyin, english " .
        ...
        while ($row = $databaseUtils->fetch_array($result)) {
            $measureWords[] = new MeasureWord($row[0], $row[1], $row[2], 
                                              $row[3], $row[4]);
        }
    ...
    function getMeasureWordsForNoun($noun) {
        ...
        $query = "SELECT id, simplified, traditional, pinyin, english, grammar, concept_cn, concept_en, " .
                 "topic_cn, topic_en, parent_cn, parent_en, measure_word " .
                 "FROM words, measure_words " .
