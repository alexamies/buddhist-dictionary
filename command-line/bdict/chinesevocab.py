...
def BuildVocabulary(directory, infile, outfile):
    ...
    wdict = _OpenDictionary() # Word dictionary

    fullpath = '%s/%s' % (directory, infile)
    if not os.path.isfile(fullpath):
        raise BDictException('%s is not a file' % infile)
    ...
    with codecs.open(full_outfile, 'w', "utf-8") as outf:
        ...
        _PrintFrequencyLinks(known_words, wdict, outf)
        outf.write('')
        outf.write('### Frequency of new words\n')
        _PrintFrequency(new_words, outf)
...
def _PrintFrequencyLinks(word_freq, sdict, outf):
...
def _PrintFrequency(word_freq, outf):
