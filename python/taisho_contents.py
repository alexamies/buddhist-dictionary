...
def convert_to_csv():
  ...
  pattern = ur"【(.*)】"
  pattern1 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]*)\s([\S]*)([譯|集|述|撰|說|記|請來|注|校|造|錄|治定|註|解|略|補|修|作|著|緝|纂|編|唱|輯])】"
  pattern2 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]*)([譯|集|述|撰|說|記|請來|注|校|造|錄|治定|註|解|略|補|修|作|著|纂|編|唱|輯])】"
  pattern3 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]+)([集|撰|造|頌|釋|作|說|論|本|糅|製|述|要|解])\s([\S]+)\s([\S]+)([譯|集|述|撰|說|記|請來|注|校|造|錄|治定|註|解|略|補|修|作|著|纂|編|唱|輯])】"
  pattern4 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]+)([集|撰|造|頌|釋|作|說|論|本|糅|製|述|要|解])\s([\S]+)[釋|論|說]\s([\S]+)\s([\S]+)([譯|集|述|撰|說|記|請來|注|校|造|錄|治定|註|解|略|補|修|作|著|纂|編|唱|輯])】"
  pattern5 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【】"
  pattern6 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【(偈本)([\S]+)\s釋論([\S]+)\s([\S]+)\s([\S]+)([譯|集|述|撰|說|記|請來|注|校|造|錄|解|略|註|補|修|作|著|纂|編|唱|輯])】"
  pattern7 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]+)\s([\S]+)([集|撰|造|頌|釋|作|說|論|本|糅|製|述|要|解|古])[\s|、|．]([\S]+)[\s|、]([\S]+)([治定|略|註|補|修|作|著|撰|述|纂|編|唱|輯])】"
  pattern8 = ur"T(\d\d)n[0]{0,3}([1-9]{1,3}[\S]*)\s([\S]*) \( (\d{1,3}) 卷\)　【([\S]+)\s([\S]+)([集|撰|造|頌|釋|作|說|論|本|糅|製|述|要|解])\s([\S]+)([譯|集|述|撰|說|記|請來|注|校|造|錄|解|略|註|補|修|作|著|纂|編|唱|輯])】"
